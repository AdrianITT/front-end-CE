  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 8. Obtener todos los registros de cotizacionServicio
      const cotizacionServicioResponse = await getAllCotizacionServicio(); // Asume que es la API correcta
      const cotizacionServicios = cotizacionServicioResponse.data;

      

        // Filtrar los registros que coincidan con el ID de la orden de trabajo
        const dataRelati = relationResponseD.data.filter(
          orden => String(orden.ordenTrabajo) === String(orderId));
      ...
      const combinedPromises = dataRelati.map(async (rel) => {
        // Obtener servicio desde la tabla "servicio"
        const servResp = await getServicioById(rel.servicio);
        const servData = servResp.data;

        // Obtener método del servicio
        const metodoResp = await getMetodoById(servData.metodos);
        const metodoData = metodoResp.data;

        // Buscar el precio en "cotizacionServicio"
        const cotizacionServicio = cotizacionServicios.find(
          (c) => c.servicio === rel.servicio && c.cotizacion === header.cotizacion
        );

        return {
          idServicio: rel.servicio,
          nombreServicio: servData.nombreServicio,
          precio: cotizacionServicio ? cotizacionServicio.precio : servData.precio, // ✅ Usa el precio correcto
